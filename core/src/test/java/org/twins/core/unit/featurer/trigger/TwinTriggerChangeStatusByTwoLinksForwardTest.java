...
class TwinTriggerChangeStatusByTwoLinksForwardTest extends BaseUnitTest {

    @Mock
    private TwinRepository twinRepository;

    private TwinTriggerChangeStatusByTwoLinksForward trigger;

    @BeforeEach
    void setUp() throws Exception {
        trigger = new TwinTriggerChangeStatusByTwoLinksForward(twinRepository);
    }
    ...
    class Run {
        ...
        void run_callsRepositoryWithCorrectParameters() throws ServiceException {
            ...
            when(twinRepository.updateTwinStatusByTwoLinksForward(
                    twin.getId(), firstLinkId, secondLinkId, classId, statusId))
                    .thenReturn(3);

            trigger.run(props, twin, null, null, null);

            verify(twinRepository).updateTwinStatusByTwoLinksForward(
                    twin.getId(), firstLinkId, secondLinkId, classId, statusId);
        }
        ...
        void run_preservesLinkIdOrder_firstThenSecond() throws ServiceException {
            ...
            inOrder.verify(twinRepository).updateTwinStatusByTwoLinksForward(
                    twin.getId(), firstLinkId, secondLinkId, classId, statusId);
        }
    }
}
