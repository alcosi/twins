...
import org.twins.core.featurer.factory.filler.FillerFieldFromContextTwinLinkedByFieldTwinField;
...
class FillerFieldFromContextTwinLinkedByFieldTwinFieldTest extends BaseUnitTest {
    ...
    private FillerFieldFromContextTwinLinkedByFieldTwinField filler;

    private static final UUID CONTEXT_TWIN_FIELD_ID = UUID.randomUUID();
    private static final UUID SRC_FIELD_ID = UUID.randomUUID();
    private static final UUID DST_FIELD_ID = UUID.randomUUID();

    @BeforeEach
    void setUp() throws Exception {
        filler = new FillerFieldFromContextTwinLinkedByFieldTwinField();
    ...
    class Fill {
        ...
        @Test
        void fill_resolvesContextLinkFieldThenLinkedTwinField() throws ServiceException {
            // NAME promises: from the context twin's link FIELD, take the linked twin, then read src field, write to dst.
            ...
            when(lookuper.lookupFieldValue(any(FactoryItemsBatch.class), eq(CONTEXT_TWIN_FIELD_ID), eq(SRC_FIELD_ID)))
            ...
            // Lookuper is invoked with both the context-twin field id and the src field id — order matters.
            verify(lookuper).lookupFieldValue(any(FactoryItemsBatch.class), eq(CONTEXT_TWIN_FIELD_ID), eq(SRC_FIELD_ID));
