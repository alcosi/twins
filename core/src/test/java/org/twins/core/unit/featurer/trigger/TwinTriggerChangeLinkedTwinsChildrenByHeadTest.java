...
class TwinTriggerChangeLinkedTwinsChildrenByHeadTest extends BaseUnitTest {

    @Mock
    private TwinRepository twinRepository;

    private TwinTriggerChangeLinkedTwinsChildrenByHead trigger;

    @BeforeEach
    void setUp() throws Exception {
        trigger = new TwinTriggerChangeLinkedTwinsChildrenByHead(twinRepository);
    }
    ...
    class Run {
        ...
        void run_callsRepositoryWithCorrectParameters() throws ServiceException {
            ...
            when(twinRepository.updateTwinStatusByLinkAndHeadTwinChildren(
                    twin.getId(), linkId, classId, statusId))
                    .thenReturn(4);

            trigger.run(props, twin, null, null, null);

            verify(twinRepository).updateTwinStatusByLinkAndHeadTwinChildren(
                    twin.getId(), linkId, classId, statusId);
        }
    }
}
