...
class TwinTriggerChangeStatusByLinkReverseTest extends BaseUnitTest {

    @Mock
    private TwinRepository twinRepository;

    private TwinTriggerChangeStatusByLinkReverse trigger;

    @BeforeEach
    void setUp() throws Exception {
        trigger = new TwinTriggerChangeStatusByLinkReverse(twinRepository);
    }
    ...
    class Run {
        ...
        void run_callsRepositoryWithCorrectParameters() throws ServiceException {
            ...
            when(twinRepository.updateTwinStatusByDstTwinIdAndLinkId(
                    twin.getId(), linkId, statusId))
                    .thenReturn(1);

            trigger.run(props, twin, null, null, null);

            verify(twinRepository).updateTwinStatusByDstTwinIdAndLinkId(
                    twin.getId(), linkId, statusId);
        }
    }
}
