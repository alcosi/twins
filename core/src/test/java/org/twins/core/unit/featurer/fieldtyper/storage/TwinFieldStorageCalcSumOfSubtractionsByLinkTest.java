...
class TwinFieldStorageCalcSumOfSubtractionsByLinkTest extends BaseUnitTest {
    ...
    private TwinFieldStorageCalcSumOfSubtractionsByLink newStorage(boolean srcElseDst, boolean statusExclude) {
    ...
    class Load {
        ...
        @Test
        void load_delegatesToSubtractionsByLinkRepository_andPacksResults() throws ServiceException {
            ...
            when(twinFieldDecimalRepository.sumLinkedTwinFieldValuesOfSubtractionsByLink(
                    ...
                    .thenReturn(List.of(
                            ...
                            new TwinFieldCalcProjection(dst2.getId(), new BigDecimal("0"))));

            storage.load(kit);

            verify(twinFieldDecimalRepository).sumLinkedTwinFieldValuesOfSubtractionsByLink(
                    eq(kit.getIdSet()), eq(true), eq(linkedInStatusIds), eq(linkedOfClassIds),
                    eq(firstFieldId), eq(secondFieldId), eq(linkIds), eq(false));
            verify(twinFieldDecimalRepository, never()).sumLinkedTwinFieldValuesOfDivisionsByLink(
                    any(), anyBoolean(), any(), any(), any(), any(), any(), anyBoolean(), anyBoolean());
            assertEquals(new BigDecimal("2"), dst1.getTwinFieldCalculated().get(fieldId));
            assertEquals(new BigDecimal("0"), dst2.getTwinFieldCalculated().get(fieldId));
        }
        ...
        void load_twinWithoutProjection_defaultsToZero() throws ServiceException {
            ...
            when(twinFieldDecimalRepository.sumLinkedTwinFieldValuesOfSubtractionsByLink(
                    ...
                    .thenReturn(List.of(new TwinFieldCalcProjection(dst1.getId(), new BigDecimal("13"))));

            storage.load(kit);

            assertEquals(new BigDecimal("13"), dst1.getTwinFieldCalculated().get(fieldId));
    ...
    class EqualsAndMerge {
        ...
        void equals_differentLinkId_isFalse() throws ServiceException {
            ...
            var b = new TwinFieldStorageCalcSumOfSubtractionsByLink(
