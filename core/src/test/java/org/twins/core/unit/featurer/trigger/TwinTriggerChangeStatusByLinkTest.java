...
class TwinTriggerChangeStatusByLinkTest extends BaseUnitTest {

    @Mock
    private TwinRepository twinRepository;

    private TwinTriggerChangeStatusByLink trigger;

    @BeforeEach
    void setUp() throws Exception {
        trigger = new TwinTriggerChangeStatusByLink(twinRepository);
    }
    ...
    class Run {
        ...
        void run_callsRepositoryWithCorrectParameters() throws ServiceException {
            ...
            when(twinRepository.updateTwinStatusByTwinClassIdAndLinkId(
                    twin.getId(), twin.getHierarchyTree(), linkId, classId, statusId))
                    .thenReturn(2);

            trigger.run(props, twin, null, null, null);

            verify(twinRepository).updateTwinStatusByTwinClassIdAndLinkId(
                    twin.getId(), twin.getHierarchyTree(), linkId, classId, statusId);
        }
    }
}
