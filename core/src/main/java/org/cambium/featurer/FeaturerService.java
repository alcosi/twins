...
public class FeaturerService {
     ...
     * {@link #toConfigKey(int, HashMap)} — order-independent and stable. Items with a {@code null}
     * featurerId are skipped.
     ...
     */
    public static <E> Collection<FeaturerParamGroup<E>> groupByFeaturerParams(
            Collection<E> items,
        ...
        Map<String, FeaturerParamGroup<E>> groups = new LinkedHashMap<>();
        for (E item : items) {
            Integer featurerId = featurerIdGetter.apply(item);
            if (featurerId == null) {
                continue;
            }
            HashMap<String, String> params = paramsGetter.apply(item);
            groups.computeIfAbsent(toConfigKey(featurerId, params),
                    key -> new FeaturerParamGroup<>(featurerId, params, key)).add(item);
        }
     ...
     * {@link #getConfigKey()} is the precomputed {@link #toConfigKey(int, HashMap)} — reuse it as a
     * cache key instead of recomputing.
     */
    public static final class FeaturerParamGroup<E> {
        private final int featurerId;
        private final HashMap<String, String> params;
        private final String configKey;
        private final List<E> items = new ArrayList<>();

        FeaturerParamGroup(int featurerId, HashMap<String, String> params, String configKey) {
        ...
        FeaturerParamGroup<E> add(E item) {
            items.add(item);
            return this;
        }
    }
