...
public class PermissionGrantUserGroupSpecification extends CommonSpecification<PermissionGrantUserGroupEntity> {
    ...
    public static Specification<PermissionGrantUserGroupEntity> checkDomainId(UUID domainId) {
        Specification<PermissionGrantUserGroupEntity>  specification = Specification.allOf(
                ...
                (root, query, cb) -> createPredicateWithJoins(root, cb, domainId, (property, criteriaBuilder, filedValue) -> criteriaBuilder.or(criteriaBuilder.isNull(property), criteriaBuilder.equal(property, filedValue)), JoinType.INNER, PermissionGrantUserGroupEntity.Fields.permissionSpecOnly, PermissionEntity.Fields.permissionGroup, PermissionGroupEntity.Fields.domainId)
        );
