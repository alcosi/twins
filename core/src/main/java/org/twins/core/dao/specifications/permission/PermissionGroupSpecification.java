...
public class PermissionGroupSpecification extends CommonSpecification<PermissionEntity> {
    ...
    public static Specification<PermissionGroupEntity> checkDomainId(UUID domainId, boolean showSystemGroup) {
        if (showSystemGroup) {
            return (root, query, cb) -> createPredicateWithJoins(root, cb, domainId, (property, criteriaBuilder, filedValue) -> criteriaBuilder.or(criteriaBuilder.isNull(property), criteriaBuilder.equal(property, filedValue)), JoinType.INNER, PermissionGroupEntity.Fields.domainId);
        } else {
