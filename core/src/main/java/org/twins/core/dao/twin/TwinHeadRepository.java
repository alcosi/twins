...
public interface TwinHeadRepository extends CrudRepository<TwinEntity, UUID>, JpaSpecificationExecutor<TwinEntity> {
    ...
    @Query(value = "select twin from TwinEntity twin join DomainBusinessAccountEntity domainBusinessAccount on twin.id = domainBusinessAccount.businessAccountId " +
            " where domainBusinessAccount.domainId = :domainId")
    Page<TwinEntity> findBusinessAccountTwinByDomainId(@Param("domainId") UUID domainId, Pageable pageable);
    @Query(value = "select twin from TwinEntity twin join DomainBusinessAccountEntity domainBusinessAccount on twin.id = domainBusinessAccount.businessAccountId  " +
            "join BusinessAccountUserEntity businessAccountUser on domainBusinessAccount.businessAccountId = businessAccountUser.businessAccountId " +
