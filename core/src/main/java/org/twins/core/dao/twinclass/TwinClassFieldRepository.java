...
public interface TwinClassFieldRepository extends CrudRepository<TwinClassFieldEntity, UUID>, JpaSpecificationExecutor<TwinClassFieldEntity> {
    ...
    @Query("select f from TwinClassFieldEntity f where f.twinClassId in :twinClassIds")
    List<TwinClassFieldEntity> findBaseFieldByIdIn(@Param("twinClassIds") Set<UUID> twinClassIds);
