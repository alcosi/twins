...
import org.twins.core.dto.rest.twinclass.TwinClassFieldValidatorUpdateRqDTOv1;
import org.twins.core.mappers.rest.mappercontext.MapperContext;
import org.twins.core.mappers.rest.related.RelatedObjectsRestDTOConverter;
import org.twins.core.mappers.rest.twinclass.TwinClassFieldValidatorRestDTOMapper;
import org.twins.core.mappers.rest.twinclass.TwinClassFieldValidatorUpdateRestDTOReverseMapper;
...
public class TwinClassFieldValidatorUpdateController extends ApiController {
    private final TwinClassFieldValidatorService twinClassFieldValidatorService;
    private final TwinClassFieldValidatorUpdateRestDTOReverseMapper twinClassFieldValidatorUpdateRestDTOReverseMapper;
    private final TwinClassFieldValidatorRestDTOMapper twinClassFieldValidatorRestDTOMapper;
    private final RelatedObjectsRestDTOConverter relatedObjectsRestDTOConverter;

    @ParametersApiUserHeaders
    @Operation(operationId = "twinClassFieldValidatorUpdateV1", summary = "Twin class field validator batch update")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Twin class field validator batch update", content = {
                    @Content(mediaType = "application/json", schema =
                    @Schema(implementation = TwinClassFieldValidatorListRsDTOv1.class))}),
            @ApiResponse(responseCode = "401", description = "Access is denied")})
    @PutMapping(value = "/private/twin_class_field_validator/v1")
    public ResponseEntity<?> twinClassFieldValidatorUpdateV1(
            ...
            @RequestBody @Valid TwinClassFieldValidatorUpdateRqDTOv1 request) {
        TwinClassFieldValidatorListRsDTOv1 rs = new TwinClassFieldValidatorListRsDTOv1();
        try {
            List<TwinClassFieldValidatorUpdate> updateList = twinClassFieldValidatorUpdateRestDTOReverseMapper.convertCollection(request.getValidators());
            Collection<TwinClassFieldValidatorEntity> entities = twinClassFieldValidatorService.updateTwinClassFieldValidators(updateList);
            rs
