...
public class DomainSubscriptionEventViewController extends ApiController {
    ...
    @Operation(operationId = "domainSubscriptionEventViewV1", summary = "Returns domain subscription event data by id")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Domain subscription event details prepared", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = DomainSubscriptionEventRsDTOv1.class))
            }),
            @ApiResponse(responseCode = "401", description = "Access is denied")})
    @GetMapping(value = "/private/domain/subscription_event/{domainSubscriptionEventId}/v1")
    public ResponseEntity<?> domainSubscriptionEventViewV1(
        ...
        try {
            DomainSubscriptionEventEntity domainSubscriptionEventEntity = domainSubscriptionEventService.findEntitySafe(domainSubscriptionEventId);
            rs
