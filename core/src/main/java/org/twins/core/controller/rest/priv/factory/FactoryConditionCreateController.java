...
@ProtectedBy(Permissions.CONDITION_SET_CREATE)
public class FactoryConditionCreateController extends ApiController {

    private final FactoryConditionService factoryConditionService;
    private final FactoryConditionCreateRestDTOReverseMapper factoryConditionCreateRestDTOReverseMapper;
    private final FactoryConditionRestDTOMapper factoryConditionRestDTOMapper;
    private final RelatedObjectsRestDTOConverter relatedObjectsRestDTOConverter;

    @ParametersApiUserHeaders
    @Operation(operationId = "factoryConditionCreateV1", summary = "Batch conditions add")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Factory condition add", content = {
                    @Content(mediaType = "application/json", schema =
                    @Schema(implementation = FactoryConditionListRsDTOv1.class))}),
            @ApiResponse(responseCode = "401", description = "Access is denied")})
    @PostMapping(value = "/private/factory_condition/v1",
    ...
    public ResponseEntity<?> factoryConditionCreateV1(
        ...
        try {
            List<TwinFactoryConditionEntity> conditions = factoryConditionService.createFactoryConditions(
                    factoryConditionCreateRestDTOReverseMapper.convertCollection(
