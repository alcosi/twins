...
public class DomainBusinessAccountDeleteController extends ApiController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "BusinessAccount was deleted", content = {
                    ...
                    @Schema(implementation = Response.class))}),
            @ApiResponse(responseCode = "401", description = "Access is denied")})
    @DeleteMapping(value = "/private/domain_business_account/{businessAccountId}/v1")
    public ResponseEntity<?> domainBusinessAccountDeleteV2(@Parameter(example = DTOExamples.BUSINESS_ACCOUNT_ID) @PathVariable UUID businessAccountId) {
        Response rs = new Response();
        try {
            domainBusinessAccountService.deleteBusinessAccountFromDomain(businessAccountId);
        } catch (ServiceException se) {
            return createErrorRs(se, rs);
        } catch (Exception e) {
            return createErrorRs(e, rs);
