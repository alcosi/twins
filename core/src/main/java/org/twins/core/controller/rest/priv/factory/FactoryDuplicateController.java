...
import org.twins.core.service.factory.FactoryDuplicateService;
...
public class FactoryDuplicateController extends ApiController {
    ...
    public ResponseEntity<?> factoryDuplicateV1(
            ...
            @Valid @RequestBody FactoryDuplicateRqDTOv1 request) {
        var rs = new FactoryListRsDTOv1();

        try {
            var duplicatedFactories = factoryDuplicateService.duplicate(factoryDuplicateRestDTOReverseMapper.convertCollection(request.duplicates, mapperContext));
            rs
