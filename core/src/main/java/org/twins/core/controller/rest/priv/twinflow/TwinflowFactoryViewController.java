...
import org.twins.core.dto.rest.twinflow.TwinflowFactoryRsDTOv1;
...
public class TwinflowFactoryViewController extends ApiController {
    ...
    private final TwinflowFactoryBaseRestDTOMapperV1 twinflowFactoryBaseRestDTOMapperV1;
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Twinflow factory dto prepared", content = {
                    ...
                    @Schema(implementation = TwinflowFactoryRsDTOv1.class))}),
            @ApiResponse(responseCode = "401", description = "Access is denied")})
    @GetMapping(value = "/private/twinflow/factory/{twinflowFactoryId}/v1")
    public ResponseEntity<?> twinflowFactoryViewV1(
            @MapperContextBinding(roots = TwinflowFactoryBaseRestDTOMapperV1.class, response = TwinflowFactoryRsDTOv1.class) @Schema(hidden = true) MapperContext mapperContext,
            @PathVariable(name = "twinflowFactoryId") UUID twinflowFactoryId) {
        TwinflowFactoryRsDTOv1 rs = new TwinflowFactoryRsDTOv1();

        try {
            rs
                    .setTwinflowFactory(twinflowFactoryBaseRestDTOMapperV1.convert(twinflowFactoryService.findEntitySafe(twinflowFactoryId), mapperContext))
