...
import org.twins.core.dto.rest.datalist.DataListSubsetCreateRqDTOv1;
import org.twins.core.dto.rest.datalist.DataListSubsetListRsDTOv1;
import org.twins.core.mappers.rest.datalist.DataListSubsetCreateDTOReverseMapper;
...
@ProtectedBy(Permissions.DATA_LIST_SUBSET_CREATE)
public class DataListSubsetCreateController extends ApiController {
    private final RelatedObjectsRestDTOConverter relatedObjectsRestDTOConverter;
    private final DataListSubsetCreateDTOReverseMapper dataListSubsetCreateDTOReverseMapper;
    ...
    @Operation(operationId = "dataListSubsetCreateV1", summary = "Data list subset create")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Data list subsets created", content = {
                    @Content(mediaType = "application/json", schema =
                    @Schema(implementation = DataListSubsetListRsDTOv1.class))}),
            @ApiResponse(responseCode = "401", description = "Access is denied")})
    @PostMapping(value = "/private/data_list_subset/v1")
    public ResponseEntity<?> dataListSubsetCreateV1(
            ...
            @RequestBody @Valid DataListSubsetCreateRqDTOv1 request) {
        DataListSubsetListRsDTOv1 rs = new DataListSubsetListRsDTOv1();
        try {
            List<DataListSubsetEntity> entities = dataListSubsetService.createDataListSubsets(dataListSubsetCreateDTOReverseMapper.convertCollection(request.getDataListSubsets()));
            rs
