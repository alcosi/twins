...
import org.twins.core.dto.rest.link.TwinLinkSearchRqDTOv1;
import org.twins.core.dto.rest.link.TwinLinkSearchRsDTOv1;
...
public class TwinLinkSearchController extends ApiController {
    ...
    @Operation(operationId = "twinLinkSearchV1", summary = "Twin link search with dynamic sorting")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Twin link data list", content = {
    ...
    public ResponseEntity<?> twinLinkSearchV1(
            @MapperContextBinding(roots = TwinLinkRestDTOMapper.class, response = TwinLinkSearchRsDTOv1.class) @Schema(hidden = true) MapperContext mapperContext,
            @SimplePaginationParams SimplePagination pagination,
            @RequestBody @Valid TwinLinkSearchRqDTOv1 request) {
        TwinLinkSearchRsDTOv1 rs = new TwinLinkSearchRsDTOv1();
        try {
            PaginationResult<TwinLinkEntity> twinLinkList = twinLinkSearchService
                    .search(twinLinkSearchDTOReverseMapper.convert(request.getSearch(), mapperContext), pagination,
                            request.getSortField(), request.getSortDirection());
            rs
                    .setPagination(paginationMapper.convert(twinLinkList))
                    .setTwinLinks(twinLinkRestDTOMapper.convertCollection(twinLinkList.getList(), mapperContext))
