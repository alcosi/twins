    ...
    private Specification<DomainUserEntity> createDomainUserSearchSpecification(DomainUserSearch search) {
        return Specification.where(
                checkBusinessAccountIn(search.getBusinessAccountIdList(), false)
                        ...
                        .and(checkDomainUserFieldLikeIn(UserEntity.Fields.name, search.getNameLikeList(), true))
                        .and(checkDomainUserFieldNotLikeIn(UserEntity.Fields.name, search.getNameNotLikeList(), true))
                        .and(checkDomainUserFieldLikeIn(UserEntity.Fields.email, search.getEmailLikeList(), true))
                        .and(checkDomainUserFieldNotLikeIn(UserEntity.Fields.email, search.getEmailNotLikeList(), true))
