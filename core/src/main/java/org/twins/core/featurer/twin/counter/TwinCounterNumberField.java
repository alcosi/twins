...
@Slf4j
@Component
@Featurer(id = FeaturerTwins.ID_5402, name = "Group by number field", description = "Count twins grouped by numeric value (stored as string, normalized via text2double)")
public class TwinCounterNumberField extends TwinCounter {
    @Override
    public GroupExpressionProvider<TwinEntity> createGroup(Properties properties, TwinClassFieldEntity twinClassFieldEntity) throws ServiceException {
        ...
        return (root, query, cb) -> {
            ...
            return cb.function("text2double", Double.class, stringValue);
        };
    }
