...
@Lazy
@Component
@Featurer(id = FeaturerTwins.ID_2722,
        name = "By status id (requested)",
        description = "")
public class TwinFinderByStatusIdRequested extends TwinFinderRequested {
    @FeaturerParam(name = "Param key", description = "", order = 1, optional = true, defaultValue = PARAM_STATUS_ID)
    ...
    public void concat(TwinSearch twinSearch, Properties properties, Map<String, String> namedParamsMap) throws ServiceException {
        Set<UUID> statusIds = getRequestedIds(paramKey, properties, namedParamsMap);
        if (CollectionUtils.isNotEmpty(statusIds)) {
            twinSearch.addStatusId(statusIds, exclude.extract(properties));
        }
    }
}
