...
public class SluggerSystemScopeDomainManage extends Slugger {
    @Autowired
    ...
    protected boolean exitGroup(Properties properties, UserEntity user, UserGroupEntity userGroup) throws ServiceException {
        var apiUser = authService.getApiUser();
        UserGroupMapEntity entityToDelete = userGroupMapRepository.findByUserIdAndUserGroupIdAndDomainId(user.getId(), userGroup.getId(), apiUser.getDomainId());
