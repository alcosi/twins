...
public class FieldFilterDependOnBooleanField extends FieldFilter{
    ...
    @FeaturerParam(name = "ShowOnFieldValue", description = "If true, filter fields by boolean field value true, if false - by field value false", order = 2, optional = true, defaultValue = "true")
    public static final FeaturerParamBoolean showOnFieldValue = new FeaturerParamBoolean("excludeOnTrue"); //TODO change in DB
    ...
    public void filterFields(Properties properties, Kit<TwinClassFieldEntity, UUID> unfilteredFieldsKit, TwinEntity twin, List<TwinClassFieldEntity> fields) throws ServiceException {
        ...
        if (booleanFieldValue == showOnFieldValue.extract(properties)) {
