...
@Featurer(id = FeaturerTwins.ID_2367,
        name = "Forward link to twin found by head and assignee",
        description = "Finds twin by head twin id and assigner_user_id matching the output twin, " +
                "then creates a forward link from output twin to the found twin. " +
                "Also searches uncommitted factory items from the current run.")
@Slf4j
public class FillerForwardLinkToTwinFoundByHeadAndAssignee extends FillerLinks {
    ...
    @FeaturerParam(name = "Resolve head root", description = "Resolve head/assignee from factory item, else from context twin", order = 4, optional = true, defaultValue = "true")
    ...
    public FillerForwardLinkToTwinFoundByHeadAndAssignee(TwinSearchServiceV2 twinSearchService) {
    ...
    public void fill(Properties properties, FactoryItem factoryItem, TwinEntity templateTwin) throws ServiceException {
        ...
        TwinEntity foundTwin = findTwin(properties, factoryItem)
                .orElseThrow(() -> new ServiceException(ErrorCodeTwins.FACTORY_PIPELINE_STEP_ERROR,
                        "Twin of class[" + twinClassId.extract(properties) + "] not found by head and assignee"));
    ...
    private Optional<TwinEntity> findTwin(Properties properties, FactoryItem factoryItem) throws ServiceException {
        ...
        if (headTwinId == null) {
            log.info("Root twin has no head, twin found by head and assignee skipped");
            return Optional.empty();
        }
        if (assigneeUserId == null) {
            log.info("Root twin has no assignee, twin found by head and assignee skipped");
            return Optional.empty();
        }
        ...
        List<TwinEntity> uncommittedMatches = findUncommittedMatches(
                factoryItem, extractedTwinClassId, headTwinId, assigneeUserId, excludeIds);
        if (uncommittedMatches.size() > 1) {
            throw new ServiceException(ErrorCodeTwins.FACTORY_PIPELINE_STEP_ERROR,
                    "More than one uncommitted twin of class[" + extractedTwinClassId + "] found by head and assignee");
        }
        if (uncommittedMatches.size() == 1) {
            return Optional.of(uncommittedMatches.getFirst());
        }
        return findSingleDbMatch(properties, factoryItem, headTwinId, assigneeUserId);
    }

    private List<TwinEntity> findUncommittedMatches(FactoryItem factoryItem, UUID extractedTwinClassId,
                                                    UUID headTwinId, UUID assigneeUserId, Set<UUID> excludeIds) {
        ...
        for (FactoryItem candidateItem : factoryItem.getFactoryContext().getAllFactoryItemList()) {
            ...
            UUID candidateHeadTwinId = candidateTwin.getHeadTwinId() != null
                    ? candidateTwin.getHeadTwinId() : candidateTwin.getId();
            if (headTwinId.equals(candidateHeadTwinId)
                    && assigneeUserId.equals(candidateTwin.getAssignerUserId())) {
    ...
    private Optional<TwinEntity> findSingleDbMatch(Properties properties, FactoryItem factoryItem,
                                                   UUID headTwinId, UUID assigneeUserId) throws ServiceException {
        ...
        PaginationResult<TwinEntity> searchResult = twinSearchService.search(
                search, new SimplePagination().setOffset(0).setLimit(2));
        ...
        if (dbMatches.size() > 1) {
            throw new ServiceException(ErrorCodeTwins.FACTORY_PIPELINE_STEP_ERROR,
                    "More than one twin of class[" + twinClassId.extract(properties) + "] found by head and assignee");
        }
        return Optional.of(dbMatches.getFirst());
    }

    private Set<UUID> buildExcludeIds(Properties properties, FactoryItem factoryItem, TwinEntity contextTwin) {
        if (!excludeFactoryInputTwin.extract(properties)) {
            return Set.of();
        }
