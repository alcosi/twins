...
public class FieldValidatorDurationEqualsDateDiff extends FieldValidatorAtomic {
    ...
    protected void beforeValidate(FieldValidateBatch batch, Properties properties) throws ServiceException {
        twinService.loadFieldsValues(batch.getTwins());
    }
    ...
    protected ValidationResult isValid(Properties properties, FieldValidateItem item) {
        ...
        FieldValue endValue = resolveValidatedOrContextValue(item, endFieldId);
    ...
    private FieldValue resolveValidatedOrContextValue(FieldValidateItem item, UUID twinClassFieldId) {
        FieldValue validatedValue = item.getValue();
        ...
        return resolveFieldValue(item.getTwinEntity(), item.getContextFields(), twinClassFieldId);
    }
}
