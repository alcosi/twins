...
public class TwinCountRestDTOMapper extends RestSimpleDTOMapper<CountResult<TwinEntity, UUID>, TwinCountDTOv1> {
    ...
    public void map(CountResult<TwinEntity, UUID> src, TwinCountDTOv1 dst, MapperContext mapperContext) throws Exception {
        ...
        if (groupFields != null) {
            if (groupFields.contains(SystemEntityService.TWIN_CLASS_FIELD_TWIN_TWIN_CLASS_ID))
                dst.setTwinClassId(entity.getTwinClassId());
            if (groupFields.contains(SystemEntityService.TWIN_CLASS_FIELD_TWIN_STATUS_ID))
                dst.setTwinStatusId(entity.getTwinStatusId());
            if (groupFields.contains(SystemEntityService.TWIN_CLASS_FIELD_TWIN_OWNER_USER_ID))
                dst.setOwnerUserId(entity.getOwnerUserId());
            if (groupFields.contains(SystemEntityService.TWIN_CLASS_FIELD_TWIN_ASSIGNEE_USER_ID))
                dst.setAssignerUserId(entity.getAssignerUserId());
            if (groupFields.contains(SystemEntityService.TWIN_CLASS_FIELD_TWIN_CREATOR_USER_ID))
        ...
        if (needLoad(mapperContext, TwinClassMode.Twin2TwinClassMode.HIDE, src, SystemEntityService.TWIN_CLASS_FIELD_TWIN_TWIN_CLASS_ID)) {
        ...
        if (needLoad(mapperContext, TwinMode.HIDE, src, SystemEntityService.TWIN_CLASS_FIELD_TWIN_HEAD_ID)) {
            twinService.loadHead(entity);
            twinBaseRestDTOMapper.postpone(entity.getHeadTwin(), mapperContext);
        }
        if (needLoad(mapperContext, UserMode.Twin2UserMode.HIDE, src, SystemEntityService.TWIN_CLASS_FIELD_TWIN_OWNER_USER_ID, SystemEntityService.TWIN_CLASS_FIELD_TWIN_CREATOR_USER_ID, SystemEntityService.TWIN_CLASS_FIELD_TWIN_ASSIGNEE_USER_ID)) {
    ...
    public void beforeCollectionConversion(Collection<CountResult<TwinEntity, UUID>> srcCollection, MapperContext mapperContext) throws Exception {
        ...
        if (needLoad(mapperContext, StatusMode.Twin2StatusMode.HIDE, someCount, SystemEntityService.TWIN_CLASS_FIELD_TWIN_STATUS_ID))
            twinService.loadStatus(entities);
        if (needLoad(mapperContext, TwinClassMode.Twin2TwinClassMode.HIDE, someCount, SystemEntityService.TWIN_CLASS_FIELD_TWIN_TWIN_CLASS_ID))
            twinService.loadClass(entities);
        if (needLoad(mapperContext, TwinMode.HIDE, someCount, SystemEntityService.TWIN_CLASS_FIELD_TWIN_HEAD_ID))
            twinService.loadHead(entities);
        if (needLoad(mapperContext, UserMode.Twin2UserMode.HIDE, someCount, SystemEntityService.TWIN_CLASS_FIELD_TWIN_OWNER_USER_ID, SystemEntityService.TWIN_CLASS_FIELD_TWIN_CREATOR_USER_ID, SystemEntityService.TWIN_CLASS_FIELD_TWIN_ASSIGNEE_USER_ID))
