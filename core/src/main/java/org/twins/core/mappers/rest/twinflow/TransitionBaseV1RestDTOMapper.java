...
public class TransitionBaseV1RestDTOMapper extends RestSimpleDTOMapper<TwinflowTransitionEntity, TwinflowTransitionBaseDTOv1> {
    ...
    public void map(TwinflowTransitionEntity src, TwinflowTransitionBaseDTOv1 dst, MapperContext mapperContext) throws Exception {
        ...
        twinflowTransitionService.loadTwinflowTransitionAlias(src);
        switch (mapperContext.getModeOrUse(TransitionMode.SHORT)) {
            case DETAILED, MANAGED -> dst
                    .setId(src.getId())
                    ...
                    .setAlias(src.getTwinflowTransitionAlias() != null ? src.getTwinflowTransitionAlias().getAlias() : null)
        ...
        if (mapperContext.hasModeButNot(StatusMode.Transition2StatusMode.HIDE)) {
            ...
            twinflowTransitionService.loadDstTwinStatus(src);
    ...
    public void beforeCollectionConversion(Collection<TwinflowTransitionEntity> srcCollection, MapperContext mapperContext) throws Exception {
        ...
        if (srcCollection.isEmpty())
            return;
        twinflowTransitionService.loadTwinflowTransitionAlias(srcCollection);
        if (mapperContext.hasModeButNot(StatusMode.Transition2StatusMode.HIDE)) {
            twinflowTransitionService.loadDstTwinStatus(srcCollection);
        }
    }
