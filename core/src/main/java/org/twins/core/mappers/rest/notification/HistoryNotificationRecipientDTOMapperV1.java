...
public class HistoryNotificationRecipientDTOMapperV1 extends RestSimpleDTOMapper<HistoryNotificationRecipientEntity, HistoryNotificationRecipientDTOv1> {
    ...
    @MapperModePointerBinding(modes = UserMode.HistoryNotificationRecipient2UserMode.class)
    ...
    public void map(HistoryNotificationRecipientEntity src, HistoryNotificationRecipientDTOv1 dst, MapperContext mapperContext) throws Exception {
        ...
        if (mapperContext.hasModeButNot(UserMode.HistoryNotificationRecipient2UserMode.HIDE)) {
            dst.setCreatedByUserId(src.getCreatedByUserId());

            historyNotificationRecipientService.loadCreatedByUser(src);
            userRestDTOMapper.postpone(src.getCreatedByUser(),
                    mapperContext.forkOnPoint(mapperContext.getModeOrUse(UserMode.HistoryNotificationRecipient2UserMode.SHORT)));
        }
    }

    @Override
    public void beforeCollectionConversion(Collection<HistoryNotificationRecipientEntity> srcCollection, MapperContext mapperContext) throws Exception {
        super.beforeCollectionConversion(srcCollection, mapperContext);
        if (mapperContext.hasModeButNot(UserMode.HistoryNotificationRecipient2UserMode.HIDE)) {
            historyNotificationRecipientService.loadCreatedByUser(srcCollection);
        }
    }
}
